...
- 2026-05-19T16:38:21.145+01:00 — A Downloads-only multiline-plus-amounts `/NeedAppearances` form initially failed in `samples/PDFFlatten.Sample` because the narrow fallback lane stopped at simple single-line widgets; the concrete blockers were one `/Ff 4096` multiline widget and five `/Q 2` amount widgets. Key paths: `samples/PDFFlatten.Sample/Program.cs`, `src/PDFFlatten/PdfFlattener.cs`
- 2026-05-19T16:45:16.216+01:00 — The narrow `/NeedAppearances` test lane now covers the Downloads-only multiline-plus-amounts shape instead of the old single-line-only limit: `tests/PDFFlatten.Tests/PdfFlattenerTests.cs` proves right-aligned single-line widgets, left-aligned multiline widgets, wrapped multiline text, and a mixed six-widget document flatten cleanly, while `tests/PDFFlatten.Tests/UnsupportedPdfGuardTests.cs` still rejects centered quadding, password-style flags, and multiline-right-aligned combinations. Key paths: `tests/PDFFlatten.Tests/PdfFlattenerTests.cs`, `tests/PDFFlatten.Tests/UnsupportedPdfGuardTests.cs`, `src/PDFFlatten/PdfFlattener.cs`
- 2026-05-19T16:45:16.216+01:00 — The real Downloads-only multiline-plus-amounts form now flattens through `samples/PDFFlatten.Sample`; flattened bytes no longer contain `/AcroForm`, `/NeedAppearances`, or `/Subtype /Widget`, which is the right structural smoke signal for this lane. Key paths: `samples/PDFFlatten.Sample/Program.cs`, `src/PDFFlatten/PdfFlattener.cs`

- 2026-05-19T16:57:22.095+01:00 — **Release gate on test3 support work (post-v0.5.0 development state):** Build ✅ clean (0 warnings), tests ✅ 84/84 passing in Release, pack ✅ succeeds, docs ✅ synced with implementation. No sensitive real-form leakage detected in tracked files. Test3 work introduces 336 lines to `src/PDFFlatten/PdfFlattener.cs` and 167 new test cases for multiline/right-aligned field support; currently uncommitted; currently version 0.5.0 (released); recommend commit as v0.5.1 before pushing to NuGet. Gate verdict: **RELEASE-READY with version decision pending**. Decision recorded at `.squad/decisions/inbox/robbie-release-gate.md`.
