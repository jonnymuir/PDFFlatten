...
- 2026-05-19T16:38:21.145+01:00 — Investigating a Downloads-only two-page empty-password Standard-security RC4-128 `/NeedAppearances` form showed the current library gets past decryption and into widget synthesis; the actual stop is the no-`/AP` multiline address widget, whose `/Ff 4096` puts it outside the supported simple single-line text lane in `src/PDFFlatten/PdfFlattener.cs`.
- 2026-05-19T16:38:21.145+01:00 — The same Downloads-only form also has five no-`/AP` right-aligned amount widgets (`/Q 2`), so compatibility would require a deliberate `/NeedAppearances` rendering expansion for multiline and alignment behavior, not a parser, encryption, or malformed-input fix.
...
- 2026-05-19T16:45:16.216+01:00 — Widened the `/NeedAppearances` text lane by one deliberate notch in `src/PDFFlatten/PdfFlattener.cs`: right-aligned single-line widgets and left-aligned multiline widgets now synthesize appearances only when layout can stay inside widget-local Helvetica-family Type1 font metrics, with simple word-wrap and fail-closed rejects for centered quadding, multiline-right-aligned combinations, width overflow, height overflow, and broader flag sets.
- 2026-05-19T16:45:16.216+01:00 — Regression coverage for that boundary now lives in `tests/PDFFlatten.Tests/PdfFlattenerTests.cs` and `tests/PDFFlatten.Tests/UnsupportedPdfGuardTests.cs`; `dotnet test --nologo` passed at 84/84, and the sample console now flattens the private Downloads-only multiline-plus-amounts form successfully with output bytes free of `/AcroForm`, `/NeedAppearances`, and `/Subtype /Widget`.
